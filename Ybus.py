...
class Ybus:
    ...
    def __init__(self, name, network: Network):
        ...
        self.name = name
        self.Y_matrix = np.zeros((len(self.network.buses), len(self.network.buses)), dtype=complex)      # creates an n by n matrix of zeros where
    ...
    def fill_ybus_matrix(self):         # fills values of Ybus matrix with necessary values
        ...
        self.Y_matrix[1][1] = self.network.transformers['T1'].y + self.network.lines['L1'].y + (self.network.lines['L1'].B/2) + self.network.lines['L2'].y + (self.network.lines['L2'].B/2)
        self.Y_matrix[1][2] = - self.network.lines['L2'].y
        self.Y_matrix[1][3] = - self.network.lines['L1'].y
        self.Y_matrix[2][1] = self.Y_matrix[1][2]
        self.Y_matrix[2][2] = self.network.lines['L2'].y + self.network.lines['L3'].y + (self.network.lines['L2'].B/2) + (self.network.lines['L3'].B/2)
        self.Y_matrix[2][4] = - self.network.lines['L3'].y
        self.Y_matrix[3][1] = self.Y_matrix[1][3]
        self.Y_matrix[3][3] = self.network.lines['L1'].y + self.network.lines['L4'].y + self.network.lines['L6'].y + (self.network.lines['L1'].B/2) + (self.network.lines['L4'].B/2) + (self.network.lines['L6'].B/2)
        ...
        self.Y_matrix[4][4] = self.network.lines['L3'].y + self.network.lines['L5'].y + self.network.lines['L6'].y + (self.network.lines['L3'].B/2) + (self.network.lines['L5'].B/2) + (self.network.lines['L6'].B/2)
        self.Y_matrix[4][5] = - self.network.lines['L5'].y
        self.Y_matrix[5][3] = self.Y_matrix[3][5]
        self.Y_matrix[5][4] = self.Y_matrix[4][5]
        self.Y_matrix[5][5] = self.network.transformers['T2'].y + self.network.lines['L4'].y + self.network.lines['L5'].y + (self.network.lines['L4'].B/2) + (self.network.lines['L5'].B/2)
